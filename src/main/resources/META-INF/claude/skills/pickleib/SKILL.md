...
description: >
  ...
  or any request to write, fix, or add to a Cucumber test in a test automation project.
...
1. **Check project setup.** Verify `Hooks.java` and `TestRunner.java` exist (with `@Pickleib` annotation). Only create or modify if missing.
